...
fn lancedb_to_bep_error(e: lancedb::Error) -> VectorStoreError {
    VectorStoreError::DatastoreError(Box::new(e))
}

fn serde_to_bep_error(e: serde_json::Error) -> VectorStoreError {
...
/// # Example
/// ```
/// use bep_lancedb::{LanceDbVectorIndex, SearchParams};
/// use bep::providers::openai::{Client, TEXT_EMBEDDING_ADA_002, EmbeddingModel};
///
...
/// # Example
/// ```
/// let search_params = bep_lancedb::SearchParams::default().distance_type(lancedb::DistanceType::Cosine);
/// ```
...
impl<M: EmbeddingModel + Sync + Send> VectorStoreIndex for LanceDbVectorIndex<M> {
    ...
    /// ```
    /// use bep_lancedb::{LanceDbVectorIndex, SearchParams};
    /// use bep::providers::openai::{EmbeddingModel, Client, TEXT_EMBEDDING_ADA_002};
    ///
    /// let openai_client = Client::from_env();
    ///
        ...
        self.build_query(query)
            ...
            .map(|(i, value)| {
                Ok((
                    ...
                    serde_json::from_value(value).map_err(serde_to_bep_error)?,
                ))
            })
            .collect()
    }
