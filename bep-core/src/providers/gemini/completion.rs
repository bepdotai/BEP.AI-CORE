...
impl completion::CompletionModel for CompletionModel {
    ...
    async fn completion(
        ...
        match response.usage_metadata {
            Some(ref usage) => tracing::info!(target: "bep",
            "Gemini completion token usage: {}",
            usage
            ),
            None => tracing::info!(target: "bep",
...
pub mod gemini_api_types {
    ...
    pub struct ContentCandidate {
        ...
        /// These are passages that are "recited" from copybephted material in the foundational LLM's training data.
    ...
    /// ### Bep Note:
    /// Can be used to cosntruct a typesafe `additional_params` in bep::[AgentBuilder](crate::agent::AgentBuilder).
    #[derive(Debug, Deserialize, Serialize)]
    #[serde(rename_all = "camelCase")]
