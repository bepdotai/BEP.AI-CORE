...
//! Bep provides a consistent API for working with LLMs and embeddings. Specifically,
...
//!
//! ## Agents
//! Bep also provides high-level abstractions over LLMs in the form of the [Agent](crate::agent::Agent) type.
//!
...
//! Bep provides a common interface for working with vector stores and indexes. Specifically, the library
...
//! # Integrations
//! ## Model Providers
//! Bep natively supports the following completion and embedding model provider integrations:
//! - OpenAI
//! - Cohere
//! - Anthropic
//! - Perplexity
...
//!
//! ## Vector Stores
//! Bep currently supports the following vector store integrations via companion crates:
//! - `bep-mongodb`: Vector store implementation for MongoDB
//! - `bep-lancedb`: Vector store implementation for LanceDB
//! - `bep-neo4j`: Vector store implementation for Neo4j
//! - `bep-qdrant`: Vector store implementation for Qdrant
//!
